...
trait TypedStringHelper[T <: TypedString] {
  ...
  implicit val jsonFormat = Format(new Reads[T] {
    ...
    override def writes(t: T): JsValue = JsString(to(t))
  })
  ...
  implicit val queryBinder = new QueryStringBindable[T] {
    override def bind(key: String, params: Map[String, Seq[String]]): Option[Either[String, T]] =
      params.get(key).map { values => values.headOption.map(v => from(v)).getOrElse(Left(buildErrMsg(values.toString))) }
    override def unbind(key: String, value: T): String = to(value)
  }
  ...
  implicit val formMapping = new Formatter[T] {
    override def bind(key: String, data: Map[String, String]): Either[Seq[FormError], T] =
      data.get(key).map { value => from(value).left.map(msg => Seq(FormError(key, msg, Nil))) }.getOrElse(Left(Seq(FormError(key, buildErrKey, Nil))))
    override def unbind(key: String, value: T): Map[String, String] = Map(key -> to(value))
  }
