...
import com.humantalks.domain.{ User, TalkData, Talk }
...
case class TalkRepository(conf: Conf, ctx: Contexts, db: Mongo) {
  ...
  def create(elt: TalkData, by: User.Id): Future[(WriteResult, Talk.Id)] = {
    val toCreate = Talk(Talk.Id.generate(), elt, Meta(new DateTime(), by, new DateTime(), by))
