  ...
  def preview = silhouette.SecuredAction(WithRole(Person.Role.Organizer)).async(parse.json) { implicit req =>
    (for {
      ...
      case Config.proposalSubmittedEmailSubject.ref => for {
        proposalOpt <- (req.body \ "id").asOpt[String].flatMap(id => Proposal.Id.from(id).right.toOption).map(id => proposalDbService.get(id)).getOrElse(proposalDbService.getLast)
      ...
      case Config.proposalSubmittedEmailContent.ref => for {
        proposalOpt <- (req.body \ "id").asOpt[String].flatMap(id => Proposal.Id.from(id).right.toOption).map(id => proposalDbService.get(id)).getOrElse(proposalDbService.getLast)
        proposalEditUrl = proposalOpt.map(p => com.humantalks.exposed.proposals.routes.ProposalCtrl.update(p.id).absoluteURL().toString).getOrElse("proposalEditUrl")
        res <- configDbService.buildProposalSubmittedEmailContent(proposalOpt, proposalEditUrl, conf.Organization.Admin.email, _ => Future.successful(value))
      } yield buildResult(res)
      case Config.proposalSubmittedSlackMessage.ref => for {
        proposalOpt <- (req.body \ "id").asOpt[String].flatMap(id => Proposal.Id.from(id).right.toOption).map(id => proposalDbService.get(id)).getOrElse(proposalDbService.getLast)
        proposalUrl = proposalOpt.map(p => com.humantalks.internal.proposals.routes.ProposalCtrl.get(p.id).absoluteURL().toString).getOrElse("proposalUrl")
        speakers <- proposalOpt.map(p => personDbService.findByIds(p.data.speakers)).getOrElse(Future.successful(List()))
        res <- configDbService.buildProposalSubmittedSlackMessage(proposalOpt, speakers, proposalUrl, _ => Future.successful(value))
      } yield buildResult(res)
      ...
      case Config.meetupCreatedSlackMessage.ref => for {
        meetupOpt <- (req.body \ "id").asOpt[String].flatMap(id => Meetup.Id.from(id).right.toOption).map(id => meetupDbService.get(id)).getOrElse(meetupDbService.getNext)
        meetupUrl = meetupOpt.map(m => com.humantalks.internal.meetups.routes.MeetupCtrl.get(m.id).absoluteURL()).getOrElse("meetupUrl")
        res <- configDbService.buildMeetupCreatedSlackMessage(meetupOpt.map(_.data), meetupUrl, _ => Future.successful(value))
      } yield buildResult(res)
      case Config.talkAddedToMeetupSlackMessage.ref => for {
        talkOpt <- (req.body \ "id").asOpt[String].flatMap(id => Talk.Id.from(id).right.toOption).map(id => talkDbService.get(id)).getOrElse(talkDbService.getLastAccepted)
        ...
        res <- configDbService.buildTalkAddedToMeetupSlackMessage(talkOpt, speakers, meetupOpt, meetupUrl, addedByOpt, _ => Future.successful(value))
      } yield buildResult(res)
