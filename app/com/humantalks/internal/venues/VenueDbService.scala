...
case class VenueDbService(venueRepository: VenueRepository, eventRepository: EventRepository) extends DbService[Venue, Venue.Id, Venue.Data, Person.Id] {
  ...
  def delete(id: Venue.Id): Future[Either[List[Event], WriteResult]] = {
    eventRepository.findForVenue(id).flatMap { events =>
      if (events.isEmpty) {
