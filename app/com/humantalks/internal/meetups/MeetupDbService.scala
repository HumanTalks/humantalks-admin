...
case class MeetupDbService(meetupRepository: MeetupRepository) extends DbService[Meetup, Meetup.Id, Meetup.Data, User.Id] {
  ...
  def create(data: Meetup.Data, by: User.Id): Future[(WriteResult, Meetup.Id)] = meetupRepository.create(data, by)
