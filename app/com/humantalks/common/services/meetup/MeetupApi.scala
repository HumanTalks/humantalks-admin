...
case class MeetupApi(conf: Conf, ctx: Contexts, ws: WSClient) {
  ...
  def createVenue(groupUrlName: String, data: VenueCreate): Future[Either[List[String], Venue]] =
    if (conf.App.isProd) {
      ws.url(s"$baseUrl/$groupUrlName/venues")
  ...
  def updateEvent(groupUrlName: String, eventId: Long, data: EventCreate): Future[Either[List[String], Event]] =
    patchEvent(groupUrlName, eventId, data.toParams)

  def announceEvent(groupUrlName: String, eventId: Long): Future[Either[List[String], Event]] =
    patchEvent(groupUrlName, eventId, Map("announce" -> "true", "status" -> "upcoming"))

  private def patchEvent(groupUrlName: String, eventId: Long, data: Map[String, String]): Future[Either[List[String], Event]] =
    if (conf.App.isProd) {
      ws.url(s"$baseUrl/$groupUrlName/events/$eventId")
  ...
  private def parseCreateVenueResponse(json: JsValue): Either[List[String], Venue] =
    (json \ "errors").asOpt[List[JsValue]].flatMap(_.flatMap(err => (err \ "potential_matches").asOpt[List[Venue]]).flatten.headOption).map(venue => Right(venue))
      .getOrElse(parseResponse(json, _.as[Venue]))
}
